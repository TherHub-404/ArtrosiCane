...
```markdown
Codex: implement this issue in `TherHub-404/ArtrosiCane`, create a branch from `main`, run the validation checks, and open a real GitHub pull request targeting `main`. Keep the PR scoped to this issue, link this Linear issue in the PR body, and comment on this issue with the GitHub PR URL. Move this Linear issue to `In Review` only after the GitHub PR exists. Do not move it to `Done`; a human reviewer will do that after review/merge. If you cannot push a branch or open a PR, leave the issue in its current state and clearly report the blocker.
```
...
6. Push the branch.
7. Open a PR to `main`.
8. Comment on the Linear issue with the GitHub PR URL and validation notes.
9. Move the Linear issue to `In Review`.

If step 6 or 7 fails, Codex should not move the issue to `In Review`; it should report the blocker instead.
