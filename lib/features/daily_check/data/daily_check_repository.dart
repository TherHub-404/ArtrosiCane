...
class DailyCheckRepository {
  ...
  Map<DailyRiskFactor, int> loadSensitivities({String? dogId}) {
    ...
    for (final factor in DailyRiskFactor.values) {
      out[factor] = (jsonMap[factor.name] as num?)?.toInt() ?? _defaultSensitivity[factor]!;
    }
    return out;
  }
  ...
  Future<void> saveDailyLog({
    ...
    await _saveRemoteLog(input, result);
  }

  Future<void> _saveLocalLog(DailyCheckInput input, DailyCheckResult result) async {
    ...
    for (final factor in DailyRiskFactor.values) {
      ...
      for (final item in entries) {
        final factors = ((item['riskFactors'] as List<dynamic>?) ?? <dynamic>[])
            .map((e) => e.toString())
            .toSet();
        final proxy = ((item['symptom'] as num?)?.toDouble() ?? 0) +
  ...
  Future<void> _saveRemoteLog(
    ...
    try {
      ...
      // Remote is optional: local persistence is the source of truth fallback.
    }
  }

  String _sensitivityKey(String? dogId) => 'factorSensitivity::${dogId ?? 'global'}';
